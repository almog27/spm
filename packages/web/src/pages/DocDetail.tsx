...
---`}</CodeBlock>
  ...
  'publishing-skills': {
    ...
    content: () => (
      <>
        <P>
          This guide covers the full publishing workflow — from preparing your skill to managing
        ...
        <UL>
          <li>
            SPM CLI installed (<Code>npm i -g @aspect/spm</Code>)
          </li>
          <li>
            Authenticated (<Code>spm login</Code>)
          </li>
          <li>
            A skill directory with a valid <Code>SKILL.md</Code>
          </li>
        </UL>

        <H2>Pre-publish checklist</H2>
        <P>Before publishing, verify your skill is ready:</P>
        <CodeBlock>{`# Run tests and security scan
spm test
...
spm info .`}</CodeBlock>
        ...
        <H2>Publishing</H2>
        <CodeBlock>{`cd my-skill/
spm publish`}</CodeBlock>
        <P>This runs the following steps automatically:</P>
        <OL>
          <li>
            Validates the <Code>SKILL.md</Code> manifest
          </li>
          <li>
            Packs all files into a <Code>.skl</Code> archive
          </li>
          <li>Signs the package with Sigstore (keyless — uses your GitHub identity)</li>
          <li>Uploads to the registry</li>
          <li>Runs the security pipeline (Layer 1 immediately, Layers 2-3 async)</li>
          <li>Classifies categories if not specified</li>
        </OL>

        <H2>Publishing new versions</H2>
        <CodeBlock>{`# Bump version first
spm version patch   # or minor, major

# Then publish
spm publish`}</CodeBlock>
        <P>
          The registry enforces that each version is unique and greater than any existing version.
          You cannot re-publish the same version number.
        </P>

        <H2>Yanking a version</H2>
        <P>
          If you publish a version with a critical bug, you can yank it. Yanked versions are hidden
          from search and won't be installed by default, but remain available for existing users who
          pin that exact version.
        </P>
        <CodeBlock>{`spm yank my-skill@1.0.0 --reason "Critical bug in prompt logic"

# ✓ Yanked my-skill@1.0.0
# Note: Existing installs with pinned versions still work`}</CodeBlock>

        <H2>Deprecating a skill</H2>
        <CodeBlock>{`# Mark as deprecated
spm deprecate my-skill --message "Use better-skill instead"

# Undo deprecation
spm deprecate my-skill --undo`}</CodeBlock>

        <H2>Transfer ownership</H2>
        <P>
          Contact the SPM team to transfer skill ownership. This is a manual process to prevent
