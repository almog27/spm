...
export const SkillNameSchema = z.string()
  .min(2).max(64)
  .regex(/^(@[a-z0-9-]+\/)?[a-z][a-z0-9-]*$/, 'Must be kebab-case, start with a letter, optionally scoped with @org/');
...
export const SkillsJsonSchema = z.object({
  ...
  resolution: z.object({
    strategy: z.enum(['project-first', 'global-first', 'strict-project', 'merge']).default('project-first'),
